...
{\centering \includegraphics[width=49.22in]{figures/xMAPr_analysis_workflow} 
...
\href{figures/inspect_single_results2_dilution_wise_range.png}{xMAPr
anaylsis output data inspection \textgreater{}\textgreater{} single
antigen/sample combination marked in global dynamic range plots}

\subsubsection{statistical analysis of calculated response data
(Screenshots)}\label{statistical-analysis-of-calculated-response-data-screenshots}

The statistical data analysis is based on samples groups defined by the
selected meta data. A wilcox rank sum test is performed using multiple
test correction if the number of groups are more than two. The test
statistics with p-value, adjusted p-value and confidence intervals etc.
can be interactively inspected and the data is plotted using a violin
plot with a bars indicating significant changes.
